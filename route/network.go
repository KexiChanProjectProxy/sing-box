	...
	nm := &NetworkManager{
		...
		defaultOptions: adapter.NetworkOptions{
			...
			DomainResolveOptions: adapter.DNSQueryOptions{
				...
				ResolveRetries: defaultDomainResolver.ResolveRetries,
				ResolveTimeout: time.Duration(defaultDomainResolver.ResolveTimeout),
				HoldValid:      time.Duration(defaultDomainResolver.HoldValid),
				HoldNX:         time.Duration(defaultDomainResolver.HoldNX),
				HoldRefused:    time.Duration(defaultDomainResolver.HoldRefused),
				HoldTimeout:    time.Duration(defaultDomainResolver.HoldTimeout),
				HoldOther:      time.Duration(defaultDomainResolver.HoldOther),
			},
			...
			TCPKeepAlive:         time.Duration(routeOptions.DefaultTCPKeepAlive),
			TCPKeepAliveInterval: time.Duration(routeOptions.DefaultTCPKeepAliveInterval),
		},
