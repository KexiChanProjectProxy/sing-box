...
每条统计记录包含 `week_start_unix` 字段，记录当前每周计费周期开始的 Unix 时间戳。这使得解析统计文件时能够将用量正确归属到对应的计费周期。

支持的模型系列包括 `gpt-5.3-codex`、`gpt-5.2-codex`、`gpt-5.1-codex`、`gpt-5-codex`、`gpt-5-codex-mini`、`gpt-5-pro`、`gpt-5`、`gpt-4.1`、`gpt-4o` 及其变体。
