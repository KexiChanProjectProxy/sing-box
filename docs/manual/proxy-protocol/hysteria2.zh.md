...
Hysteria 2 是一个基于 QUIC 的简洁中国制造协议。
其卖点是 Brutal，一种即使在丢包情况下也能尝试达到用户定义带宽的拥塞控制算法。

!!! warning

    尽管 GFW 很少封锁基于 UDP 的代理，但这类协议实际上比基于 TCP 的代理具有更明显的特征。
...
它本质上是将 `<username>:<password>` 组合作为实际密码，
而 sing-box 不提供此别名。
要使 sing-box 与官方程序配合使用，您需要将该组合作为实际密码填入。
