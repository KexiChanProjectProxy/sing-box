...
长期以来，图形操作系统中代理客户端的现代使用方式和原理一直没有被清晰描述。
不过，我们可以将它们分为三类：系统代理、防火墙重定向和虚拟接口。
...
作为最知名的代理方式之一，它有许多缺点：
许多非基于 HTTP 的 TCP 客户端不会检查和使用系统代理。
此外，UDP 和 ICMP 流量会绕过代理。

```mermaid
flowchart LR
    dns[DNS 查询] -- 是 HTTP 请求？ --> proxy[HTTP 代理]
    dns --> leak[泄漏]
    tcp[TCP 连接] -- 是 HTTP 请求？ --> proxy
...
虽然这种方式的侵入性较强且配置繁琐，
但由于对软件的技术要求较低，它在 V2Ray 等业余代理开源项目社区中仍然很受欢迎。
